...
import * as service from "./carriers.service";

export async function list(req: FastifyRequest, rep: FastifyReply) {
  const companyId = req.auth!.companyId;
  const q = CarrierListQuerySchema.parse(req.query ?? {});
...
export async function create(req: FastifyRequest, rep: FastifyReply) {
  const companyId = req.auth!.companyId;
  const body = CarrierCreateSchema.parse(req.body);
...
export async function update(req: FastifyRequest, rep: FastifyReply) {
  const companyId = req.auth!.companyId;
  const { id } = IdParamSchema.parse(req.params);
  const body = CarrierUpdateSchema.parse(req.body);
