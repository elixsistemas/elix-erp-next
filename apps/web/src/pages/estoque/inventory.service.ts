import type { InventoryMovementRow, InventoryStockRow, ProductMini, MovementType } from "./inventory.types";
...
// ✅ endpoints via proxy do Vite (/api -> backend)
...
export function getStock(productId: number) {
  ...
  return api<{ productId: number; onHand: number }>(`${invBase}/stock?${usp.toString()}`);
}

// MOVIMENTOS
export function listMovements(params: { productId?: number; type?: MovementType; limit?: number; offset?: number }) {
...
export function createMovement(payload: MovementFormValues) {
  // mantém compat e já manda campos "futuros" sem quebrar o form
  ...
  return api<{ ok: true; deduped?: boolean; movement?: any }>(movBase, {
    method: "POST",
    body: JSON.stringify(finalPayload),
  });
}

// MVP: lista simples de produtos (pra exibir nome e popular selects)
