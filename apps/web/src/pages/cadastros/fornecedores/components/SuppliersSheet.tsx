...
import { type SupplierUpsertForm } from "../suppliers.schema";

import { onlyDigits } from "@/shared/br/digits";
import { maskCPF, maskCNPJ, maskCep, maskPhoneBR } from "@/shared/br/masks";
...
type Props = {
  ...
  onSubmit: (data: SupplierUpsertForm) => Promise<void> | void;
};

const empty: SupplierUpsertForm = {
  name: "",
  document: "",
...
export function SupplierSheet({ open, onOpenChange, mode, saving, initialData, onSubmit }: Props) {
  const [tab, setTab] = React.useState("geral");
  const [sameAsBilling, setSameAsBilling] = React.useState(true);
  const [form, setForm] = React.useState<SupplierUpsertForm>(empty);
  ...
  React.useEffect(() => {
    ...
    if (mode === "edit" && initialData) {
      const next: SupplierUpsertForm = {
        ...empty,
        ...pickSupplierToForm(initialData),
      };
      setForm(next);
  ...
  function set<K extends keyof SupplierUpsertForm>(key: K, value: SupplierUpsertForm[K]) {
    setForm((prev) => {
  ...
  function setDigits<K extends keyof SupplierUpsertForm>(key: K, raw: string) {
  ...
  async function lookupShippingCep() {
    ...
    try {
      ...
      toast.success("Endereço carregado pelo CEP (Retirada)");
    } catch (e: any) {
      toast.error(e?.message ?? "Falha ao consultar CEP");
    }
  }
  ...
  async function lookupCnpj() {
    ...
    try {
      ...
      // Preenche cobrança como base (você pode trocar p/ retirada se quiser)
      if (j?.logradouro) {
  ...
  const maskedDoc =
    personType === "PF"
      ? maskCPF(form.document ?? "")
      : maskCNPJ(form.document ?? "");
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-[860px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>
            {mode === "create" ? "Novo fornecedor" : `Editar fornecedor #${initialData?.id ?? ""}`}
          </SheetTitle>
        </SheetHeader>
          ...
          <Tabs value={tab} onValueChange={setTab}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onBlur={async () => {
                        if (personType === "PJ") await lookupCnpj();
                      }}
                    />
                    <Button
                      type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Dados úteis para compras/financeiro (prazo, contato, regras, etc.)"
                  />
                </div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="cobranca" className="mt-4 space-y-4">
              <AddressForm
                ...
                set={set}
                setDigits={setDigits}
                onLookupCep={lookupBillingCep}
              />
            </TabsContent>

            <TabsContent value="retirada" className="mt-4 space-y-4">
              <div className="flex items-center justify-between border rounded-lg p-3">
                <div>
                  <div className="font-medium">Retirada igual cobrança</div>
                  <div className="text-sm text-muted-foreground">
                    Ative para manter sincronizado e evitar retrabalho.
                  </div>
                </div>
              ...
              <div className={sameAsBilling ? "opacity-60 pointer-events-none" : ""}>
                <AddressForm
                  title="Endereço de retirada/coleta"
                  prefix="shipping"
                  get={form}
                  set={set}
                  setDigits={setDigits}
                  onLookupCep={lookupShippingCep}
                />
              </div>
            </TabsContent>
          </Tabs>
...
function AddressForm(props: {
  ...
  title: string;
  get: SupplierUpsertForm;
  set: <K extends keyof SupplierUpsertForm>(key: K, value: SupplierUpsertForm[K]) => void;
  setDigits: <K extends keyof SupplierUpsertForm>(key: K, raw: string) => void;
  onLookupCep: () => void;
}) {
  const p = props.prefix;
  const key = (name: string) => `${p}_${name}` as keyof SupplierUpsertForm;
      ...
      <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="space-y-2">
          ...
          <Button
            ...
            size="icon"
            title="Buscar CEP"
            onClick={() => void props.onLookupCep()}
          >
            <Search className="h-4 w-4" />
          </Button>
        ...
        <div className="space-y-2">
          ...
          <Input
            value={v("state")}
            onChange={(e) => props.set(key("state"), e.target.value as any)}
          />
        </div>
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <Input
            ...
            onChange={(e) => props.set(key("address_line1"), e.target.value as any)}
          />
        </div>
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <Input
            ...
            onChange={(e) => props.set(key("address_line2"), e.target.value as any)}
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            value={v("district")}
            onChange={(e) => props.set(key("district"), e.target.value as any)}
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            value={v("city")}
            onChange={(e) => props.set(key("city"), e.target.value as any)}
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            value={v("country") || "BR"}
            onChange={(e) => props.set(key("country"), e.target.value as any)}
          />
        </div>
      </div>
    </div>
...
function pickSupplierToForm(s: Supplier): SupplierUpsertForm {
  return {
    name: s.name ?? "",
    document: onlyDigits(s.document ?? ""),

    person_type: (s.person_type as any) ?? null,
    ie: s.ie ?? null,

    email: s.email ?? null,
    phone: onlyDigits(s.phone ?? ""),
    mobile: onlyDigits(s.mobile ?? ""),
    contact_name: s.contact_name ?? null,

    notes: s.notes ?? null,

    billing_address_line1: s.billing_address_line1 ?? null,
    billing_address_line2: s.billing_address_line2 ?? null,
    billing_district: s.billing_district ?? null,
    billing_city: s.billing_city ?? null,
    billing_state: s.billing_state ?? null,
    billing_zip_code: onlyDigits(s.billing_zip_code ?? ""),
    billing_country: s.billing_country ?? "BR",

    shipping_address_line1: s.shipping_address_line1 ?? null,
    shipping_address_line2: s.shipping_address_line2 ?? null,
    shipping_district: s.shipping_district ?? null,
    shipping_city: s.shipping_city ?? null,
    shipping_state: s.shipping_state ?? null,
    shipping_zip_code: onlyDigits(s.shipping_zip_code ?? ""),
    shipping_country: s.shipping_country ?? "BR",
  };
}

function syncShippingFromBilling(
  current: SupplierUpsertForm,
  setForm: React.Dispatch<React.SetStateAction<SupplierUpsertForm>>
) {
  setForm((prev) => ({
    ...prev,
