...
import type { PagedResult, ListFiscalQuery, Cest } from "./fiscal.types";
import * as api from "./fiscal.service";

export function useFiscalCest() {
  const [query, setQuery] = useState<ListFiscalQuery>({ page: 1, pageSize: 25, active: "1" });
  const [data, setData] = useState<PagedResult<Cest> | null>(null);
  ...
  const reload = async () => {
    ...
    setError(null);
    try {
      const res = await api.listCest(query);
      setData(res);
    } catch (e: any) {
      setError(e?.message ?? "Erro ao carregar CEST");
    } finally {
      setLoading(false);
    }
  };
