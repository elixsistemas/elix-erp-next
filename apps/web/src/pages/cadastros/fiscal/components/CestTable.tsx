...
import { useFiscalCest } from "../useFiscalCest";
import * as api from "../fiscal.service";
import { CestFormSchema } from "../fiscal.schema";
...
export function CestTable() {
  const { data, loading, error, query, setQuery, paging, reload } = useFiscalCest();
  ...
  async function onSave(form: any) {
    const parsed = CestFormSchema.safeParse(form);
    if (!parsed.success) {
      alert(parsed.error.issues.map((i) => i.message).join("\n"));
      return;
    }
    if (editing?.id) await api.updateCest(editing.id, parsed.data);
    else await api.createCest(parsed.data as any);
    setEditing(null);
    await reload();
  }

  async function onToggle(id: number) {
    await api.toggleCest(id);
    await reload();
  }
  ...
  async function handleImportFile(file: File) {
    setImporting(true);
    try {
      const res = await api.importCestFile(file, true);
      setPreview({
        file,
        fileName: file.name,
      ...
      toast.error(e?.message ?? "Falha ao importar arquivo de CEST");
    } finally {
      setImporting(false);
    }
  }
  ...
  async function confirmImport() {
    ...
    setImporting(true);
    try {
      const res = await api.importCestFile(preview.file, false);
      toast.success(`CEST importado: +${res.inserted} / ~${res.updated}`);
      setPreview(null);
      await reload();
    } catch (e: any) {
      toast.error(e?.message ?? "Falha ao executar import de CEST");
    } finally {
      setImporting(false);
    }
  }
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-end">
        <div className="flex flex-col">
          ...
          <input
            ...
            placeholder="Código, descrição ou segmento…"
          />
        </div>
        ...
        <button
          ...
          onClick={() => setEditing({ code: "", description: "", segment: null, active: true })}
          disabled={importing}
        >
          Novo CEST
        </button>
        ...
        <button
          ...
          disabled={importing}
          title="Importar CEST via XLSX/XLS (com prévia dry-run)"
        >
      ...
      <div className="border rounded-md overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="text-left p-3 w-[110px]">Código</th>
              <th className="text-left p-3">Descrição</th>
              <th className="text-left p-3 w-[260px]">Segmento</th>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td className="p-3" colSpan={5}>Carregando…</td>
              </tr>
            ) : items.length === 0 ? (
              <tr>
                <td className="p-3" colSpan={5}>Nada encontrado.</td>
              </tr>
            ) : (
              items.map((it) => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white w-full max-w-xl rounded-lg p-4 space-y-3">
            ...
            <div className="flex justify-end gap-2">
              <button className="px-4 py-2 rounded-md border" onClick={() => setPreview(null)} disabled={importing}>
                Cancelar
              </button>
              <button className="px-4 py-2 rounded-md bg-black text-white" onClick={() => void confirmImport()} disabled={importing}>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-lg p-4 space-y-3">
            <div className="font-semibold">{editing.id ? "Editar CEST" : "Novo CEST"}</div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditing((s: any) => ({
                      ...s,
                      segment: e.target.value.trim() === "" ? null : e.target.value,
                    }))
                  }
                />
              </div>
