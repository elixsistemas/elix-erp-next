...
import { useFiscalCfop } from "../useFiscalCfop";
import * as api from "../fiscal.service";
import { CfopFormSchema } from "../fiscal.schema";
...
export function CfopTable() {
  const { data, loading, error, query, setQuery, paging, reload } = useFiscalCfop();
  ...
  async function onSave(form: any) {
    const parsed = CfopFormSchema.safeParse(form);
    if (!parsed.success) {
      alert(parsed.error.issues.map((i) => i.message).join("\n"));
      return;
    }
    if (editing?.id) await api.updateCfop(editing.id, parsed.data);
    else await api.createCfop(parsed.data as any);
    setEditing(null);
    await reload();
  }

  async function onToggle(id: number) {
    await api.toggleCfop(id);
    await reload();
  }
    ...
    setImporting(true);
    try {
        const res = await api.importCfopFile(file, true);
        setPreview({
        file,
        fileName: file.name,
        ...
        toast.error(e?.message ?? "Falha ao importar arquivo de CFOP");
    } finally {
        setImporting(false);
    }
    }
    ...
    setImporting(true);
    try {
        const res = await api.importCfopFile(preview.file, false);
        toast.success(`CFOP importado: +${res.inserted} / ~${res.updated}`);
        setPreview(null);
        await reload();
    } catch (e: any) {
        toast.error(e?.message ?? "Falha ao executar import de CFOP");
    } finally {
        setImporting(false);
    }
    }
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-end">
        ...
        <button
          ...
          onClick={() => setEditing({ code: "", description: "", nature: null, active: true })}
          disabled={importing}
        >
          Novo CFOP
        </button>
        ...
        disabled={importing}
        title="Importar CFOP via XLSX/XLS/CSV/JSON (com prévia dry-run)"
        >
      ...
      <div className="border rounded-md overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="text-left p-3 w-[100px]">Código</th>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-lg p-4 space-y-3">
            <div className="font-semibold">{editing.id ? "Editar CFOP" : "Novo CFOP"}</div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditing((s: any) => ({
                      ...s,
                      nature: e.target.value === "" ? null : Number(e.target.value),
                    }))
                  }
                />
              </div>
