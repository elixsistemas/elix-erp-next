...
export function useFiscalNcm() {
  const [query, setQuery] = useState<ListFiscalQuery>({ page: 1, pageSize: 25, active: "1" });
  const [data, setData] = useState<PagedResult<Ncm> | null>(null);
  ...
  const reload = async () => {
    ...
    setError(null);
    try {
      const res = await api.listNcm(query);
      setData(res);
    } catch (e: any) {
      setError(e?.message ?? "Erro ao carregar NCM");
    } finally {
      setLoading(false);
    }
  };
